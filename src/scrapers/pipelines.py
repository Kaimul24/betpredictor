    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item, OddsItem):
            ...
            execute_query("""
                ...
                (name, normalized_player_name, player_id, season, month, frv, total_innings, innings_C, innings_1B, innings_2B, 
                 innings_3B, innings_SS, innings_LF, innings_CF, innings_RF)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (p['name'], normalized_name, p['player_id'], p['season'], p['month'], p['frv'], p['total_innings'], p['innings_C'],
