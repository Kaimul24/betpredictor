...
class FeaturePipeline():
    ...
    def _match_schedule_to_odds(self, schedule_data: DataFrame, odds_data: DataFrame) -> DataFrame:
        ...
        # Only keep the aggregated odds features generated by odds.py, not raw sportsbook data
        ...
        # Group by game and take first for metadata (should be same across sportsbooks)
        metadata_df = merged_games.groupby(level=id_cols)[existing_metadata_cols].first()
        
        if existing_odds_cols:
            # Take first aggregated odds features for each game (they should be the same across sportsbooks)
